...
def target_select_and_extend(name_set, ques, answ, orig_template, template_seeds, thres):
    ...
    #print(f'target length: {len(target_coins)}')
    ...
    False_label = False_ds[:len(True_label)]
    
    # ground_truth:
    ground_truth = [1 for _ in range(len(True_label))] + [0 for _ in range(len(False_label))]
    # Test Group:
    test_group = True_label + False_label
...
for thres in tqdm(thres_group):
    ft = open(f'true_data_{thres}', 'wb')
    ff = open(f'false_data_{thres}', 'wb')
    ...
    for i in tqdm(range(100)):
        ...
        true_result, false_result = target_select_and_extend(subname_set, ques, answ, orig_target, template_cluster(), thres)
